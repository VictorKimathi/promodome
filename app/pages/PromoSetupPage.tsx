...
const PromoSetupPage: React.FC<PromoSetupPageProps> = ({ onPromoComplete }) => {
  ...
  const [finalNumberOfRounds, setFinalNumberOfRounds] = useState<number | null>(null);
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8 flex flex-col items-center">
        ...
        <Card className="w-full max-w-3xl shadow-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
          <CardHeader className="text-center p-6 border-b border-gray-200 dark:border-gray-700">
            <h1 className="text-3xl font-bold tracking-[0.3em] text-sky-400">Dev info-PAGE LAYOUT</h1>
            <h1 className="text-5xl font-bold tracking-wider mb-1">PROMO</h1>
            <h2 className="text-4xl font-bold tracking-[0.3em]">DOME</h2>
          </CardHeader>
          ...
          <CardContent className="p-6 space-y-5">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Button
                variant="outline"
                className={`text-white font-semibold px-4 py-2 rounded ${roundsMode !== 'manual' ? 'bg-blue-500 hover:bg-blue-600' : 'bg-green-500 hover:bg-green-600'}`}
            ...
            <div className="text-right text-xs text-gray-500 dark:text-gray-400 pt-2">
              {isMounted && currentTime ? formatDateTime(currentTime) : 'Loading time...'}
            </div>
          </CardContent>
          ...
          <CardFooter className="flex flex-col sm:flex-row justify-between items-center gap-4 p-6 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 rounded-b-lg">
            <Button
              ...
              className="w-full sm:w-auto bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg transition duration-300 ease-in-out transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center relative"
            >
              <Crown className="left-2 top-1/2 -translate-y-1/2 absolute h-5 w-5 text-yellow-400" />
              {isProcessing ? (
                <>
              ...
              <Crown className="right-2 top-1/2 -translate-y-1/2 absolute h-5 w-5 text-yellow-400" />
            </Button>
            <img
